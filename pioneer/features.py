...
def calculate_conservation(seq_dict, non_cached_dir):
    ...
    for prot, sequence in seq_dict.items():
        ...
        if not os.path.exists(msa_file):
            print('PSIBLAST search failed for %s' % prot)
            continue
        if not uniprot_seq_dict:
            with open(UNIPROT_SEQ_PICKLE_PATH, 'rb') as f:
                uniprot_seq_dict = pickle.load(f)
                
        cdhit_input_file = format_rawmsa(prot, sequence, msa_file, uniprot_seq_dict, os.path.join(non_cached_dir, 'msa'))
        if cdhit_input_file is None or not os.path.exists(cdhit_input_file):
            continue
...
def calculate_coevolution(int_list, seq_dict, non_cached_dir):
    ...
    for prot in prots:
        if os.path.exists(os.path.join(non_cached_dir, 'msa', prot+'_rawmsa.fasta')):
            ...
            msa_fasta_file = format_rawmsa(prot, sequence, msa_file, uniprot_seq_dict, os.path.join(non_cached_dir, 'msa'))
            if msa_fasta_file is None or not os.path.exists(msa_fasta_file):
                continue
        msa_fasta_file_dict[prot] = msa_fasta_file
        
    for i in int_list_:
        if not (i[0] in msa_fasta_file_dict and i[1] in msa_fasta_file_dict):
            continue
...
def calculate_excluded_pdb_dict(int_list, seq_dict, non_cached_dir):
    ...
    if os.path.exists(os.path.join(homolog_dir, 'query.fasta')):
        os.system('%s -query %s -db %s -num_threads 12 -outfmt 6 > %s' % (BLASTP, os.path.join(homolog_dir, 'query.fasta'), os.path.join(homolog_dir, 'db', 'sifts.fasta'), os.path.join(homolog_dir, 'blastp_output.txt'))) # PARAM
