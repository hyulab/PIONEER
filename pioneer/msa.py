...
def format_rawmsa(prot_id, sequence, msa_file, seq_dict, out_dir):
    ...
    if len(identifiers_to_align) > 0:
        with open(os.path.join(out_dir, prot_id+'_rawmsa.fasta'), 'w') as f:
            for identifier in identifiers_to_align:
                f.write('>' + identifier + '\n' + seq_dict[identifier.split('|')[1]] +'\n')
        return os.path.join(out_dir, prot_id+'_rawmsa.fasta')
    else:
        return None
