...
class KAF(nn.Module):
    ...
    def __init__(self, num_parameters, D=20, conv=False, boundary=4.0, init_fcn=None, kernel='gaussian'):
        ...
        # Save the dictionary
        if self.conv:
            self.register_buffer('dict', torch.from_numpy(self.dict_numpy).view(1, 1, 1, 1, 1, -1))
            self.unsqueeze_dim = 5
        else:
        ...
        if kernel == 'gaussian':
            ...
            # Initialize gamma
            if self.conv:
                self.register_buffer('gamma', torch.from_numpy(np.ones((1, 1, 1, 1, 1, self.D), dtype=np.float32)*self.gamma_init))
            else:
        ...
        if self.conv:
            self.alpha = Parameter(torch.FloatTensor(1, self.num_parameters, 1, 1, 1, self.D))
        else:
