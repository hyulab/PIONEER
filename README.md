...
Please enter the root folder of the github repository, and execute the following command:
```
python run_pioneer.py test_interactions.txt result_folder_name
```
The results can be found in the user_results folder. To avoid repeatedly calculating the features for the same inputs in your next running, you can copy the calculated features from each feature folder in user_results/result_folder_name/non_cached_files folder over to the corresponding cache directory indicated in pioneer/config.py.
